...
#define ___define_ht_set_r(key_t, val_t) \
	void key_t##_##val_t##_ht_set_r(key_t##_##val_t##_ht_t *ht, key_t * key, val_t * val);
#define ___define_ht_set_r_impl(key_t, val_t) \
	void key_t##_##val_t##_ht_set_r(key_t##_##val_t##_ht_t *ht, key_t * key, val_t * val) {	\
		const uint64_t powers_of_2[] = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216, 33554432, 67108864, 134217728, 268435456, 536870912, 1073741824, 2147483648, 4294967296};	\
		size_t index = key_t##_ht_hash(*key) % ht->size;	\
		___ht_entry_##key_t##_##val_t* current_entry = &ht->entries[index];	\
...
#define ___define_ht_dump(key_t, val_t) \
	void key_t##_##val_t##_ht_dump(key_t##_##val_t##_ht_t *ht);

#define ___define_ht_dump_impl(key_t, val_t) \
	void key_t##_##val_t##_ht_dump(key_t##_##val_t##_ht_t *ht){	\
		for (int i = 0; i < ht->size; i++) {	\
			___ht_entry_##key_t##_##val_t * cur = &ht->entries[i];	\
...
#define ___define_ht_removeAll(key_t, val_t) \
	void key_t##_##val_t##_ht_removeAll(key_t##_##val_t##_ht_t *ht);

#define ___define_ht_removeAll_impl(key_t, val_t) \
	void key_t##_##val_t##_ht_removeAll(key_t##_##val_t##_ht_t *ht){	\
		if(!ht && ht->length != 0) return;		\
		key_t##_##val_t##_ht_destroy(ht);	\
		*ht = key_t##_##val_t##_ht_create();	\
	}

#define ___define_ht_ht_merge(key_t, val_t) \
	void key_t##_##val_t##_ht_merge(key_t##_##val_t##_ht_t *ht0, key_t##_##val_t##_ht_t *ht1);

#define ___define_ht_ht_merge_impl(key_t, val_t) \
	void key_t##_##val_t##_ht_merge(key_t##_##val_t##_ht_t *ht0, key_t##_##val_t##_ht_t *ht1){	\
		if(!ht0 || !ht1) return;	\
		ht_foreach(ht0, pair, i, key, val)	\
			key_t##_##val_t##_ht_set(key, val);	\
		ht_foreach_end()	\
	}
...
#define define_ht_all(key_t, val_t) \
	...
	___define_ht_dump(key_t, val_t)	\
	___define_ht_removeAll(key_t, val_t)	\
	___define_ht_ht_merge(key_t, val_t)	\
...
#define define_ht_all_impl(key_t, val_t)	\
	...
	___define_ht_dump_impl(key_t, val_t)	\
	___define_ht_removeAll_impl(key_t, val_t)	\
	___define_ht_ht_merge_impl(key_t, val_t)	\
